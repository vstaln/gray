...
while IFS= read -r line; do
  case "$line" in
    *plugin/manifest*)
      ...
      printf '{"id":%s,"result":{"name":"echo","version":"0.1.0","tools":[{"name":"echo","description":"Echo text back","parameters":{"type":"object","properties":{"text":{"type":"string"}},"required":["text"]},"snippet":"echo <text>"}],"commands":["/echo"],"hooks":["turn/end"]}}\n' "$id"
      ;;
    *command/run*)
      id=$(printf '%s' "$line" | sed 's/.*"id":\([0-9]*\).*/\1/')
