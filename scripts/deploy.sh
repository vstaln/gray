...
SSH "sudo mkdir -p /var/www/gray/dl/.staging/$CHANNEL \
     && sudo install -m 644 /tmp/$BASE /var/www/gray/dl/.staging/$CHANNEL/$BASE \
     && sudo cp /var/www/gray/dl/.staging/$CHANNEL/$BASE /var/www/gray/dl/$BASE \
     && sudo install -m 755 /tmp/gray-install-$UNIQ.sh /var/www/gray/install.sh \
     && rm -f /tmp/$BASE /tmp/gray-install-$UNIQ.sh"

# version manifest for the in-app update check — swapped LAST, after the
# staged tarball is promoted live (cf release.yml finalize job).
...
for f in "gray-$CHANNEL-x86_64-linux.tar.gz" "gray-$CHANNEL-aarch64-linux.tar.gz" "gray-$CHANNEL-aarch64-darwin.tar.gz" "gray-$CHANNEL-x86_64-darwin.tar.gz" "latest-$CHANNEL.txt"; do
