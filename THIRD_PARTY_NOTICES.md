...
below because gray's ACP support is built on it. Everything else below is a
port or parity target with a source comment at the use site. Studied-but-unused
`reference/` checkouts (herdr, openclaw, continue, crawlee, aider, cline,
t3code, empryo, …) are NOT listed: no code or design taken, so no attribution
owed. If a verbatim port ever lands, its file keeps the upstream license
header and the full text ships alongside.
