...
async fn s1_background_wait_exit() -> Row {
    ...
    let bg = call(
        ...
        &BashTool,
        &ctx,
        json!({"command": "echo bg1; sleep 2", "background": true}),
    )
    .await;
    ...
    let done = call(
        ...
        &ShellOutputTool,
        &ctx,
        json!({"task_id": id, "wait": "exit", "timeout": 10}),
    )
    .await;
    ...
    Row {
        n: 1,
        name: "background + wait=exit",
        calls: m.calls,
        chars: m.chars,
        max_single: m.max_single,
        wall_ms: t0.elapsed().as_millis(),
        note: "2 calls, zero polls".into(),
    }
}
...
async fn s3_benign_stops() -> Row {
    ...
    assert!(!out.is_error, "{}", out.content);
    assert!(
        out.content.contains("no matches — not an error"),
        "{}",
        out.content
    );
    ...
    Row {
        n: 3,
        name: "benign grep miss → stop",
        calls: m.calls,
        chars: m.chars,
        max_single: m.max_single,
        wall_ms: t0.elapsed().as_millis(),
        note: "1 call, not an error".into(),
    }
}
...
async fn s4_promotion() -> Row {
    ...
    let out = call(
        ...
        &BashTool,
        &ctx,
        json!({"command": "echo tick; sleep 3", "timeout": 1}),
    )
    .await;
    assert!(!out.is_error, "{}", out.content);
    let head = out.content.lines().next().unwrap_or("").to_string();
    ...
    Row {
        n: 4,
        name: "timeout → promotion",
        calls: m.calls,
        chars: m.chars,
        max_single: m.max_single,
        wall_ms: t0.elapsed().as_millis(),
        note: "2 calls, process survived".into(),
    }
}
...
async fn s5_cursor_reads() -> Row {
    ...
    let bg = call(
        ...
        &BashTool,
        &ctx,
        json!({"command": "echo first; sleep 2; echo second; sleep 1", "background": true}),
    )
    .await;
    assert!(!bg.is_error, "{}", bg.content);
    let id = format!("t{}", task_n(&bg.content, "started t"));
    ...
    let again = call(
        ...
        &ShellOutputTool,
        &ctx,
        json!({"task_id": id, "from_offset": at}),
    )
    .await;
    ...
    Row {
        n: 5,
        name: "cursor reads, no re-reads",
        calls: m.calls,
        chars: m.chars,
        max_single: m.max_single,
        wall_ms: t0.elapsed().as_millis(),
        note: format!("offsets 0→{at}→exit"),
    }
}
...
async fn s6_monitor_no_poll() -> Row {
    ...
    let bg = call(
        ...
        &BashTool,
        &ctx,
        json!({"command": "sleep 2; echo done-eval6", "background": true}),
    )
    .await;
    assert!(!bg.is_error, "{}", bg.content);
    let id = format!("t{}", task_n(&bg.content, "started t"));
    ...
    Row {
        n: 6,
        name: "monitor, zero poll turns",
        calls: m.calls,
        chars: m.chars,
        max_single: m.max_single,
        wall_ms: t0.elapsed().as_millis(),
        note: "2 calls, no sleep+tail loop".into(),
    }
}
...
async fn s7_bounded_spew() -> Row {
    ...
    assert!(!out.is_error, "{}", out.content);
    assert!(
        out.content.len() <= 55 * 1024,
        "scenario 7 regressed: {} bytes",
        out.content.len()
    );
    assert!(
    ...
    let logged = std::fs::read_to_string(log_path_of(out.content.lines().next().unwrap_or("")))
        .expect("log exists");
    assert_eq!(logged.bytes().filter(|&b| b == b'\n').count(), 30_000);
    Row {
        n: 7,
        name: "30k-line spew bounded",
        calls: m.calls,
        chars: m.chars,
        max_single: m.max_single,
        wall_ms: t0.elapsed().as_millis(),
        note: format!("{} bytes ≤ 55 KiB", out.content.len()),
    }
}
...
async fn s8_no_dup_bytes() -> Row {
    ...
    let bg = call(
        ...
        &BashTool,
        &ctx,
        json!({"command": "echo line-A; sleep 1; echo line-B; sleep 1", "background": true}),
    )
    .await;
    assert!(!bg.is_error, "{}", bg.content);
    ...
    assert!(
        b1.contains("line-A") && !b1.contains("line-B"),
        "pages disjoint: {b1:?}"
    );
    assert!(
        b2.contains("line-B") && !b2.contains("line-A"),
        "pages disjoint: {b2:?}"
    );
    ...
    Row {
        n: 8,
        name: "paged reads disjoint",
        calls: m.calls,
        chars: m.chars,
        max_single: m.max_single,
        wall_ms: t0.elapsed().as_millis(),
        note: format!("{total}B vs {unique}B unique ({ratio:.2}×)"),
    }
}
...
async fn s9_exit_data_and_fence() -> Row {
    ...
    .await;
    assert!(
        fe.content.contains("<\\/untrusted-output"),
        "{}",
        fe.content
    );
    assert_eq!(
        fe.content.matches("<untrusted-output").count(),
        1,
        "{}",
        fe.content
    );
...
async fn s10_kill() -> Row {
    ...
    loop {
        if matches!(
            registry().get(&session, TaskId(n)).map(|i| i.state),
        ...
        assert!(
            ...
            "task t{n} not reaped after kill"
        );
        tokio::time::sleep(Duration::from_millis(100)).await;
    }
    Row {
        n: 10,
        name: "kill stops group",
        calls: m.calls,
        chars: m.chars,
        max_single: m.max_single,
        wall_ms: t0.elapsed().as_millis(),
...
async fn eval_measured_scenarios() {
    ...
    let mut md = String::from(
        "| scenario | calls | chars (~tokens) | wall | note |\n|---|---|---|---|---|\n",
    );
