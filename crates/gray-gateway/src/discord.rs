...
pub struct DiscordAdapter { token: String }
impl DiscordAdapter { pub fn new(cfg: PlatformConfig) -> anyhow::Result<Self> { Ok(Self{token:cfg.token.ok_or_else(|| anyhow::anyhow!("discord token not set"))?}) } }
#[async_trait::async_trait] impl BasePlatformAdapter for DiscordAdapter {
