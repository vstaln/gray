...
async fn disabled_profile_sidecars_warn_and_skip_before_spawn() {
    ...
    save(false, false);
    let (plugins, fallback) = active_plugins(default_plugins(), &["tools-basic", "tools-search"], profile_str.as_str(), None, true)
        .await
        .unwrap();
    ...
    save(true, true);
    let (plugins, _) = active_plugins(default_plugins(), &["tools-basic", "tools-search"], profile_str.as_str(), None, false)
        .await
        .unwrap();
    let warnings = take_builder_warnings();
    assert!(manifest_names(&plugins).contains(&"echo".to_string()));
...
async fn lock_install_dir_activates_and_respects_enabled_flag() {
    ...
    let (plugins, fallback) =
        active_plugins(default_plugins(), &["tools-basic", "tools-search"], missing_profile.as_str(), None, false)
            .await
            .unwrap();
    ...
    save_user(true);
    let (plugins, _) = active_plugins(default_plugins(), &["tools-basic", "tools-search"], missing_profile.as_str(), None, false)
        .await
        .unwrap();
    assert!(
        manifest_names(&plugins).contains(&"echo".to_string()),
