...
mod tests {
    use super::*;
    use crate::store::{CronJob, CronStorePaths};
    use tempfile::TempDir;

    fn temp_store(tag: &str) -> (TempDir, CronStorePaths) {
        let dir = TempDir::new().unwrap();
        let store = CronStorePaths::from_home(dir.path());
        (dir, store)
    }

    fn job_at(id: &str, schedule: &str, next_run: Option<DateTime<Utc>>) -> CronJob {
        let mut j = CronJob::new(id.to_string(), schedule.to_string(), "prompt".into());
        j.id = id.to_string();
        j.next_run = next_run;
        j
    }
    ...
    #[test]
    fn scan_fires_within_grace() {
        let (_d, store) = temp_store("due");
        let sched = Scheduler::new(store.clone());
        let now = Utc::now();
        let job = job_at("due1", "every 60m", Some(now - chrono::Duration::seconds(600)));
        // persist
        {
            let jobs = vec![job];
            let body = serde_json::to_string_pretty(&jobs).unwrap();
            std::fs::create_dir_all(&store.cron_dir).unwrap();
            std::fs::write(&store.jobs_file, body).unwrap();
        }
        let due = sched.scan_due_jobs().unwrap();
        assert_eq!(due.len(), 1);
        assert_eq!(due[0].id, "due1");
    }

    #[test]
    fn scan_future_not_due() {
        let (_d, store) = temp_store("future");
        let sched = Scheduler::new(store.clone());
        let job = job_at("later", "every 60m", Some(Utc::now() + chrono::Duration::seconds(1800)));
        {
            let jobs = vec![job];
            std::fs::create_dir_all(&store.cron_dir).unwrap();
            std::fs::write(&store.jobs_file, serde_json::to_string_pretty(&jobs).unwrap()).unwrap();
        }
        assert!(sched.scan_due_jobs().unwrap().is_empty());
    }

    #[test]
    fn inflight_blocks_double_fire() {
        let guard = InflightGuard::new();
        assert!(guard.try_register_running_job("j1"));
        assert!(!guard.try_register_running_job("j1"));
        guard.release_running_job("j1");
        assert!(guard.try_register_running_job("j1"));
    }
}
