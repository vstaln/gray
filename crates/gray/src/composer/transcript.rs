...
mod tests {
    ...
    #[test]
    fn batch_insert_before_equals_per_line() {
        let w = 9usize;
        let line = Line::from("hello world hello"); // 17 chars, wrapping at 9 => 2+ lines with word wrap
        let wrapped = wrap_styled_line(line.clone(), w);
        // t1: batch
        let mut t1 = Terminal::new(TestBackend::new(10, 10)).unwrap();
        let batch_lines = wrapped.clone();
        let h = batch_lines.len() as u16;
        let _ = t1.insert_before(h, |buf| {
            Paragraph::new(batch_lines.clone()).render(buf.area, buf);
        });
        // t2: per-line inserts
        let mut t2 = Terminal::new(TestBackend::new(10, 10)).unwrap();
        for l in wrapped.clone() {
            let _ = t2.insert_before(1, |buf| {
                Paragraph::new(l).render(buf.area, buf);
            });
        }
        // Compare underlying buffers - they should have same content height
        // TestBackend doesn't expose scrollback, but we can compare that batch height equals sum of per-line inserts count
        assert_eq!(h as usize, wrapped.len());
        // Ensure wrapping preserved words (not splitting "hello" mid-word when possible)
        for l in &wrapped {
            assert!(l.width() <= w, "wrapped line width {} exceeds max {}", l.width(), w);
        }
        // Check no mid-word split for simple case "a b c" at width 3 should be 2 lines: "a b" + "c" or "a" + "b c" but not "a b" split inside word
        let simple = Line::from("a b c");
        let w_simple = wrap_styled_line(simple, 3);
        // Should not contain "a b" split mid-word; acceptable variations but ensure no chunk exceeds width and words preserved
        assert!(w_simple.len() >= 2);
        for l in &w_simple {
            assert!(l.width() <= 3);
        }
        let txt: String = w_simple.iter().map(|l| l.spans.iter().map(|s| s.content.as_ref()).collect::<String>()).collect::<Vec<_>>().join("|");
        // Should contain whole words, not "a " + "b" split incorrectly
        assert!(txt.contains('a') && txt.contains('b') && txt.contains('c'));
    }

    #[test]
    fn word_wrap_does_not_split_mid_word() {
        let line = Line::from("hello world foo bar");
        let wrapped = wrap_styled_line(line, 11); // "hello world" is 11, should stay together
    ...
    #[test]
    fn wrap_respects_has_bg_and_hyperlink_guards() {
        use ratatui::style::Color;
        let line_bg = Line::from(Span::styled("hello world hello world", Style::default().bg(Color::Red)));
        let wrapped_bg = wrap_styled_line(line_bg, 5);
        assert_eq!(wrapped_bg.len(), 1); // not wrapped
        let line_link = Line::from(Span::raw("\x1b]8;;http://example.com\x07link\x1b]8;;\x07 hello world"));
        let wrapped_link = wrap_styled_line(line_link, 5);
        assert_eq!(wrapped_link.len(), 1);
    }

    #[test]
    fn long_word_broken_when_exceeds_width() {
