...
pub(crate) use widgets::{
    build_input_box, composer_content_h, queued_preview_h, queued_preview_lines, shimmer_spans,
    status_dock_h, thinking_style, transcript_ends_blank,
};
...
pub(crate) fn draw(tui: &mut Tui) -> anyhow::Result<()> {
    ...
    // Exact-fit viewport: grow/shrink to content so no cleared slack sits
    // under the footer. ponytail: panel estimated (PANEL_ROWS / capped
    // matches); draw() caps to avail anyway.
    //
    // Shrink is two-phase (codex parity: closing the slash popup must not
    // leave ghost rows). Recreating an `Inline(h)` viewport prints `h-1`
    // newlines and abandons the old rows as scrollback, so shrinking
    // directly strands the old popup/footer rows on screen (double footer,
    // stale matches, ever-growing scrollback). Instead the loop below first
    // paints the small content into the outgoing viewport — surplus rows are
    // `Clear`ed — and only then shrinks and repaints, abandoning blanks.
    let mut shrink_to: Option<u16> = None;
    {
        let queued_h = queued_preview_h(tui.queued_inputs.len());
        let panel_est: u16 = if question_active {
            PANEL_ROWS as u16
        } else {
            tui.matches.len().min(PANEL_ROWS) as u16
        };
        let desired = composer_content_h(status_h, queued_h, box_h, panel_est, attach_h)
    ...
    // repaints the shrunken one. `.map(|_| ())` drops the frame so no
    // terminal borrow survives into the recreate between passes.
