...
pub(crate) fn draw(tui: &mut Tui) -> anyhow::Result<()> {
    ...
    // Fixed-height viewport (baseline Inline(VIEWPORT_H) behavior): no
    // exact-fit grow/shrink. The filler bottom-anchor + surplus Clear below
    // stay live every frame, so unpainted rows are cleared, never ghosts.
    ...
    // Single-pass draw into the fixed-height viewport.
    let res: std::io::Result<()> = tui
        .terminal
        .draw(|frame| {
            let area = frame.area();
            ...
            let need = if question_active {
                tui.active_question
                    .as_ref()
                    .map(|q| {
                        super::question::panel_lines(q, tui.textarea.text(), w, 100).len() as u16
                    })
