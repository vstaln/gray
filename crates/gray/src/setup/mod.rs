...
pub use catalog::{
    ...
    load_catalog, load_saved_config_at, mask_key_pretty, normalize_auth_mode, save_saved_config_at,
    saved_config_path,
};
                ...
                match &mut state {
                    ModalState::Selecting => {
                        ...
                        if filtered.is_empty() {
                            ...
                            frame.render_widget(
                                empty_msg,
                                Rect::new(inner.x, list_y + 1, inner.width, 1),
                            );
                        } else {
                            ...
                            for r in 0..list_h {
                                ...
                                let fill = (inner.width as usize)
                                    .saturating_sub(raw_content.chars().count());
                    ...
                    ModalState::EnteringKey {
                        ...
                        frame.render_widget(
                            Paragraph::new(input_content).style(Style::default().bg(input_bg)),
                            input_rect,
                        );
                    ...
                    ModalState::ChoosingAuthMethod {
                        ...
                        for (r, label) in rows.iter().enumerate() {
                            ...
                            frame.render_widget(
                                Paragraph::new(line),
                                Rect::new(inner.x, inner.y + 2 + r as u16, inner.width, 1),
                            );
                        }
                        ...
                        KeyCode::Enter => {
                                ...
                                crate::oauth::oauth_provider_for_connect_id(&item.id)
                            {
                                ...
                                match auth_result {
                                    ...
                                    Err(e) => {
                                        let short: String =
                                            format!("{e:#}").chars().take(100).collect();
    ...
    let result = (|| -> anyhow::Result<Option<String>> {
        loop {
            terminal.draw(|frame| {
                ...
                let modal_h = (providers.len() as u16 + 6)
                    .min(area.height.saturating_sub(2))
                    .max(10)
                    .min(area.height);
                let modal_x = (area.width.saturating_sub(modal_w)) / 2;
                ...
                for (idx, (id, display)) in providers.iter().enumerate() {
                    let is_selected = idx == sel;
                    let is_auth = crate::proxy::get_adapter(id)
                    ...
                    let row_line = if is_selected {
                        ...
                        let status_span =
                            Span::styled(status, Style::default().fg(status_color).bg(box_bg));
                        let pad_span = Span::styled(" ".repeat(fill), Style::default().bg(box_bg));
