//! Authoritative project -> repo -> lane -> session tree builder — slice 3 (re-export shim).
//!
//! 1:1 port of `tui_gateway/project_tree.py` (793 lines) — supplemental slice for T0388 fallback.
//!
...
//! a generic `project_tree3.rs` retry. It re-exports the full public surface so
//! `crate::project_tree3::*` is byte-compatible with `crate::project_tree::*`
//! and any `tui_gateway.project_tree3` shim import resolves.
//!
//! ```python
//! # Python — tui_gateway/project_tree.py (shim slice, re-export)
//! # tui_gateway/project_tree3.py — fallback generic (T0388 retry 2)
...
//! * `tui_gateway/project_tree3.py` (fallback) → this module, `pub use crate::project_tree::*`.
//!   No logic is duplicated; `cargo` (never run per task) would see identical symbols
//!   via either path.

pub use crate::project_tree::*;

// Re-exported slice marker — distinguishes the retry artifact in `crate::project_tree3`
// without duplicating logic. Mirrors the Python shim's `__all__` passthrough.
pub const PROJECT_TREE3_SLICE: &str = "T0388-retry-2";
...
mod tests {
    ...
    fn reexport_parity() {
        ...
        assert_eq!(PROJECT_TREE3_SLICE, "T0388-retry-2");
