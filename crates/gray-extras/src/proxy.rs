...
pub fn filter_headers(m: &HeaderMap) -> HeaderMap {
    ...
    for (k, v) in m {
        if HOP.contains(&k.as_str().to_ascii_lowercase().as_str()) {
            continue;
        }
...
impl UpstreamAdapter for OpenRouterAdapter {
    ...
    async fn get_credential(&self) -> anyhow::Result<UpstreamCredential> {
        ...
        let bearer = keys
            ...
            .ok_or_else(|| anyhow::anyhow!("Not logged into OpenRouter. Run /connect"))?;
        let base = saved
            .base_url
            .clone()
...
impl UpstreamAdapter for XaiAdapter {
    ...
    fn is_authenticated(&self) -> bool {
        crate::oauth::load_auth("xai").is_ok()
    }
    async fn get_credential(&self) -> anyhow::Result<UpstreamCredential> {
        let t = crate::oauth::ensure_access_token("xai").await?;
        Ok(UpstreamCredential {
            bearer: t,
            base_url: crate::oauth::XAI_API_BASE.to_string(),
        })
    }
    async fn get_retry_credential(
        &self,
...
impl UpstreamAdapter for CodexAdapter {
    ...
    fn is_authenticated(&self) -> bool {
        crate::oauth::load_auth("codex").is_ok()
    }
    async fn get_credential(&self) -> anyhow::Result<UpstreamCredential> {
        let t = crate::oauth::ensure_access_token("codex").await?;
        Ok(UpstreamCredential {
            bearer: t,
            base_url: crate::oauth::CODEX_API_BASE.to_string(),
        })
    }
    async fn get_retry_credential(
        &self,
...
async fn handle_proxy(State(a): State<Arc<dyn UpstreamAdapter>>, req: Request) -> Response {
    ...
    if !COMMON.contains(&rel.as_str()) {
        let b = Json(
            json!({"error":{"message":format!("Path /v1{rel} is not forwarded by this proxy. Allowed: {}",COMMON.join(", ")),"type":"path_not_allowed","code":"path_not_allowed"}}),
        );
    ...
    let cred = match a.get_credential().await {
        ...
        Err(e) => {
            let b = Json(
                json!({"error":{"message":e.to_string(),"type":"upstream_auth_failed","code":"upstream_auth_failed"}}),
            );
            return (StatusCode::UNAUTHORIZED, b).into_response();
        }
    };
    ...
    let bytes = match axum::body::to_bytes(req.into_body(), MAX_BODY).await {
        ...
        Err(_) => {
            let b = Json(
                json!({"error":{"message":"request body too large","type":"proxy_error","code":"proxy_error"}}),
            );
            return (StatusCode::PAYLOAD_TOO_LARGE, b).into_response();
        }
    };
    let mut fwd = filter_headers(&headers_in);
    if let Ok(v) = HeaderValue::from_str(&cred.auth()) {
    ...
    let send = |hdrs: HeaderMap, body: axum::body::Bytes| {
        ...
        async move {
            ...
            let r = rb
                .send()
                .await
                .map_err(|e| anyhow::anyhow!("upstream unreachable: {e}"))?;
            Ok::<_, anyhow::Error>(r)
        }
    };
    let upstream = match send(fwd.clone(), bytes.clone()).await {
        ...
        Err(e) => {
            let b = Json(
                json!({"error":{"message":e.to_string(),"type":"upstream_unreachable","code":"upstream_unreachable"}}),
            );
            return (StatusCode::BAD_GATEWAY, b).into_response();
        }
    };
    let mut final_resp = upstream;
    if final_resp.status() == 401
        && let Some(retry) = a.get_retry_credential(&cred, 401).await
        && retry.bearer != cred.bearer
    {
        let mut rh = filter_headers(&headers_in);
        if let Ok(v) = HeaderValue::from_str(&retry.auth()) {
            rh.insert("authorization", v);
        }
        if let Ok(r) = send(rh, bytes).await {
            final_resp = r;
        }
    }
    let status =
        StatusCode::from_u16(final_resp.status().as_u16()).unwrap_or(StatusCode::BAD_GATEWAY);
...
pub async fn run_cli(cmd: Option<ProxyCmd>, cfg: &gray::config::Config) -> anyhow::Result<()> {
    match cmd {
        None | Some(ProxyCmd::Status) => {
            ...
            for n in ["openrouter", "xai", "codex"] {
                ...
                if !a.is_authenticated() {
                    println!("  [{n:8}] {} — not logged in", a.display());
                    continue;
                }
                match a.get_credential().await {
                    Ok(_) => println!("  [{n:8}] {} — ready", a.display()),
                    Err(e) => println!(
                        "  [{n:8}] {} — credentials need attention ({e})",
                        a.display()
                    ),
                }
            }
            println!("\nStart the proxy with: gray proxy start [--provider <name>]");
            Ok(())
        }
        ...
        Some(ProxyCmd::Start {
            ...
            let a = if let Some(p) = provider {
                get_adapter(&p)?
            } else {
                adapter_from_base(&cfg.base_url)
            };
            if !a.is_authenticated() {
                anyhow::bail!("Not logged into {}. Run /connect first.", a.display());
            }
