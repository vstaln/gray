...
- `GatewaySessionStore` trait `get_or_create(key,src)->SessionId` impl `FileGatewayStore` `~/.gray/gateway_sessions.json` `RwLock<HashMap>` atomic replace fsync.
