...
- The crate is named `gray pulse`; binary is `gray pulse`.
...
- Every task ends with `cargo test -p gray pulse` green and a commit.
...
```bash
git add Cargo.toml crates/gray pulse
...
Run: `cargo test -p gray pulse job`
Expected: FAIL.
...
- [ ] **Step 3: Run to verify failure** — `cargo test -p gray pulse enable` → FAIL.
...
Run: `cargo test -p gray pulse && cargo run -p gray pulse -- status`
...
- [ ] **Step 2: Run to verify failure** — `cargo test -p gray pulse plugin` → FAIL.
