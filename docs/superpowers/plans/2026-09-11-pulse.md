...
> **Status:** this plan documents the original standalone crate, later folded
> into the core `gray` crate as the `gray pulse` subcommand. `cargo build -p …`
> commands below that target a separate crate are historical; pulse now builds
> as part of `gray`.

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Give gray a 24/7 pulse — a standing goal the agent works on a schedule, reaching the user on chat only when there is something to say — as a gated `gray pulse` crate that never touches `gray-core`.

**Architecture:** Reuse `gray_cron` for scheduling and the gateway's existing cron ticker + `[SILENT]`-aware delivery. Pulse owns a goal file (`$GRAY_HOME/pulse/goal.md`), a config file (`$GRAY_HOME/pulse.json`), and one cron job named `pulse`. A standalone `gray pulse` binary manages it; the same binary runs as a sidecar plugin exposing a `pulse` tool.

**Tech Stack:** Rust 2024, `anyhow`, `clap`, `serde`/`serde_json`, `chrono`, `gray-cron`; stdio (no tokio).

**Spec:** `docs/superpowers/specs/2026-09-11-pulse-design.md`

## Global Constraints

- Implemented as a core `gray` module (`crates/gray/src/pulse/`) exposed as the `gray pulse` subcommand in the default build — no separate crate, no separate binary.
- It reuses `gray-cron` and `gray-gateway::config::gray_home_dir` (both already deps of `gray`) and adds no new dependency to the `gray` crate.
- Config file is JSON at `$GRAY_HOME/pulse.json`; goal at `$GRAY_HOME/pulse/goal.md`; cron job name is exactly `pulse`.
...
- Every task ends with `cargo test -p gray` green and a commit.
...
**Files:**
- Create: `crates/gray pulse/Cargo.toml`
- Create: `crates/gray pulse/src/lib.rs`
- Create: `crates/gray pulse/src/config.rs`
- Create: `crates/gray pulse/src/goal.rs`
...
- Produces:
  ...
  - `pub struct PulseConfig { pub enabled: bool, pub schedule: String, pub deliver: String }` (Default: enabled false, schedule `"every 30m"`, deliver `"local"`)
  - `pub fn config_path() -> anyhow::Result<PathBuf>`; `pub fn load_config() -> anyhow::Result<PulseConfig>`; `pub fn save_config(&PulseConfig) -> anyhow::Result<()>`
...
In root `Cargo.toml`: add `"crates/gray pulse",` to `members` (NOT `default-members`), and under `[workspace.dependencies]` add:
```toml
gray pulse = { path = "crates/gray pulse" }
```
...
description = "gray's 24/7 pulse — a standing goal run on a schedule"
...
Create `crates/gray pulse/src/goal.rs` tests and `config.rs` tests. In `config.rs`:
```rust
#[cfg(test)]
mod tests {
    use super::*;
    ...
    fn config_round_trips_and_defaults() {
        ...
        let cfg = PulseConfig { enabled: true, schedule: "every 1h".into(), deliver: "telegram:1".into() };
...
`lib.rs`:
```rust
//! gray pulse: a standing goal run on a schedule by the gateway cron ticker.
pub mod config;
pub mod goal;
...
```
`config.rs`: serde struct as in Interfaces, `config_path()` = `gray_home()?.join("pulse.json")`, `load_config()` returns Default when the file is absent, `save_config` writes pretty JSON (create parent dirs).
`goal.rs`: `goal_path()` = `gray_home()?.join("pulse").join("goal.md")`, `read_goal()` returns `""` when absent, `write_goal` creates parent dirs and writes.
...
```bash
git add Cargo.toml crates/gray/src/pulse
git commit -m "feat(pulse): crate skeleton, config and goal file"
```

---

### Task 2: Pulse prompt + cron job sync

**Files:**
- Create: `crates/gray pulse/src/job.rs`
- Modify: `crates/gray pulse/src/lib.rs` (add `pub mod job;`)

**Interfaces:**
- Consumes: `config::{PulseConfig, load_config}`, `goal::read_goal`, `gray_cron`.
- Produces:
  ...
  - `pub fn sync_job(cfg: &PulseConfig, goal: &str) -> anyhow::Result<String>` — removes any existing job named `pulse`, and if `cfg.enabled`, adds one with `schedule=cfg.schedule`, `deliver=Deliver::Target(cfg.deliver)`, `prompt=render_prompt(goal)`; returns the job id or `""` when disabled.
...
mod tests {
    ...
    fn sync_adds_updates_and_removes_job() {
        ...
        let dir = tempfile::tempdir().unwrap();
        unsafe { std::env::set_var("GRAY_HOME", dir.path()) };
        let mut cfg = PulseConfig { enabled: true, schedule: "every 30m".into(), deliver: "local".into() };
    ...
    fn status_reports_disabled_missing_and_live() {
        ...
        let cfg = PulseConfig { enabled: true, schedule: "every 30m".into(), deliver: "local".into() };
...
```bash
git add crates/gray/src/pulse
git commit -m "feat(pulse): prompt template and cron job sync"
```

---

### Task 3: `gray pulse` CLI binary

**Files:**
- Create: `crates/gray pulse/src/main.rs`
- Modify: `crates/gray pulse/Cargo.toml` (add `[[bin]]`)

**Interfaces:**
- Consumes: `config::*`, `goal::*`, `job::*`.
- Produces: binary `gray pulse` with subcommands:
...
Add to `crates/gray pulse/Cargo.toml`:
```toml
[[bin]]
name = "gray pulse"
path = "src/main.rs"
...
CLI behavior is thin over the lib; test the one non-trivial helper. Add to `job.rs` (or a `cli.rs` module) `pub fn enable(cfg: &mut PulseConfig, schedule: Option<String>, deliver: Option<String>) -> anyhow::Result<String>` that applies overrides, sets `enabled = true`, saves config, and syncs the job. Test:
```rust
#[test]
...
- [ ] **Step 3: Run to verify failure** — `cargo test -p gray enable` → FAIL.

- [ ] **Step 4: Implement `enable` + `main.rs`**

```rust
pub fn enable(cfg: &mut PulseConfig, schedule: Option<String>, deliver: Option<String>) -> anyhow::Result<String> {
...
Run: `cargo test -p gray && cargo run -p gray -- pulse status`
...
```bash
git add crates/gray/src/pulse
git commit -m "feat(pulse): gray pulse CLI (on/off/status/goal/sync)"
```
...
**Files:**
- Create: `crates/gray pulse/src/plugin.rs`
- Modify: `crates/gray pulse/src/lib.rs` (`pub mod plugin;`), `src/main.rs` (handle `--plugin` before subcommands)

**Interfaces:**
- Consumes: `config`, `goal`, `job`.
- Produces: `pub fn serve_plugin() -> anyhow::Result<()>` — reads newline-delimited JSON on stdin, writes replies on stdout, exposing one tool `pulse` with `action` ∈ `{status, goal_get, goal_set, on, off, sync}` (+ optional `text`, `schedule`, `deliver`), and answers `plugin/manifest`. Exits 0 on `plugin/shutdown`.
...
- [ ] **Step 2: Run to verify failure** — `cargo test -p gray plugin` → FAIL.
...
```bash
cargo test -p gray
printf '%s\n' '{"id":1,"method":"plugin/manifest"}' | cargo run -q -p gray -- pulse plugin
```
Expected: PASS; smoke prints a manifest JSON line naming the `pulse` tool.

- [ ] **Step 5: Commit**

```bash
git add crates/gray/src/pulse
git commit -m "feat(pulse): sidecar plugin mode (pulse tool)"
```
...
**Files:**
- Modify: `README.md` (a short "Pulse" subsection under Gateway)
- Create: `docs/pulse.md`

- [ ] **Step 1: Write `docs/pulse.md`** covering: what it is, `gray pulse on --every 30m --deliver telegram:123`, goal file, `[SILENT]` quieting, that the gateway daemon must be running, and how to register it as a plugin:
```yaml
plugins:
  - tools-minimal
  - sidecar: [gray, pulse, plugin]
```
- [ ] **Step 2: Add a 3-line README pointer** under the Gateway section linking `docs/pulse.md`.
- [ ] **Step 3: Verify links/format** — `cargo fmt --all --check`; read both docs.
- [ ] **Step 4: Commit**
```bash
git add README.md docs/pulse.md
git commit -m "docs(pulse): usage, goal file, plugin registration"
```
...
- Type consistency: `PulseConfig`, `sync_job`, `render_prompt`, `JOB_NAME`, `job_status`/`JobStatus`, `enable`, `handle_line` are used consistently across tasks.
